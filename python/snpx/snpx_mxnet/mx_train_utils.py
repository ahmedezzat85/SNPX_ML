...
def mx_create_data_iterator(batch_size, train_set_file=None, val_set_file=None, 
        ...
        if train_set_file is not None:
            train_iter = MxImageIter(batch_size=batch_size, data_shape=shape, path_imgrec=train_set_file)
        if val_set_file is not None:
            val_iter = MxImageIter(batch_size=batch_size, data_shape=shape, path_imgrec=val_set_file)
...
class MxImageIter(mx.io.DataIter):
    ...
    def __init__(self, batch_size, data_shape, label_width=1, path_imgrec=None, 
        ...
        super(SPXImageIter, self).__init__()
...
# VIDEO DATA ITERATOR   
#----------------------------------------------------#
class SNPXVideoIter(mx.io.DataIter):
    """Image data iterator with a large number of augumentation choices.
    Supports reading from both .rec files and raw image files with image list.
    ...
    label_width : int
        dimension of label
    """
    def __init__(self, batch_size, data_shape, label_width=1, path_root=None, vid_list=None, shuffle=False):
        # Validate the data shape
        #assert len(data_shape) == 3 and data_shape[0] == 3
        super(SNPXVideoIter, self).__init__()

        self.seq            = np.arange(len(vid_list), dtype=int)
        self.shuffle        = shuffle
        self.batch_size     = batch_size
        self.data_shape     = data_shape
        self.label_width    = label_width
        ...
        #TODO: CHECK THIS --> # Set data and label shapes
        self.provide_data = [('data', (batch_size,) + data_shape)]
        if label_width > 1:
            self.provide_label = [('softmax_label', (batch_size, label_width))]
        else:
    ...
    def read_video_file(self, vfile_path, new_size=None):
        """
        """
        cap = cv2.VideoCapture(vfile_path)
        while(cap.isOpened()):
            ret, img = cap.read()
            if not ret:
                break
            ...
            img = nd.array(img)
            vid.append(cv2.resize(im, (171, 128)))

    def next_sample(self):
        """helper function for reading in next sample"""
        if self.curr_video is None:
            # Read the next video file
            if self.idx >= len(self.seq):
                raise StopIteration
            idx = self.seq[self.idx]
            self.idx += 1
            root, vfile, label = self.vid_list[idx]
            vid_file = os.path.join(self.path_root, root, vfile)
        else:
            pass
